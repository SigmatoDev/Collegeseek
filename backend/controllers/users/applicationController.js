...
const s3 = require("../../utils/s3"); // ✅ use shared config
...
const saveFile = async (file, fieldName) => {
  ...
  if (isImage) {
    ...
    if (compressedBuffer.length > IMAGE_MAX) {
      throw new Error(
        `Image too large for ${fieldName} after compression. Max 5MB.`
      );
    }

    const key = `applications/${timestamp}-${fieldName}.jpg`;
  ...
  const key = `applications/${timestamp}-${fieldName}.${ext}`;
