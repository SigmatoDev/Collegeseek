...
export default function AdoutMetaEditor() {
  ...
  useEffect(() => {
    const fetchMeta = async () => {
      try {
        const res = await axios.get(`${api_url}aboutget/meta?page=about`);
  ...
  const handleSave = async () => {
    try {
      await axios.post(`${api_url}aboutupdate/meta`, form);
  ...
  return (
    <div className="max-w-[1600px] mx-auto mt-10 bg-white border border-gray-200 shadow-lg rounded-xl p-8 space-y-6">
      <h2 className="text-2xl font-bold text-gray-800">Meta Editor - AboutUs Page</h2>
