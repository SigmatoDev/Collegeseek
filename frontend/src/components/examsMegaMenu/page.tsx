...
export default function ExamMegaMenu() {
  ...
  useEffect(() => {
    ...
    const fetchMenu = async () => {
      ...
      try {
        const res = await fetch(`${api_url}exam-menus`, {
          signal: controller.signal,
        });
        if (!res.ok) throw new Error("Failed to load exam menus");
        const menus = await res.json();
        const examMenu = menus.data?.[0];
        if (examMenu?.columns && Array.isArray(examMenu.columns)) {
          setMenuData(examMenu.columns);
        }
      } catch (error: any) {
        if (error?.name !== "AbortError") {
          console.warn("Exam menu is unavailable. Using empty exam menu.");
        }
      }
    };
    ...
    <>
      <div className="relative" ref={megaMenuRef}>
        ...
        >
          ...
          <span className="md:hidden">Exams</span>
          ...
          <div className="w-[220px] shrink-0 bg-[#fafafa] border-r border-gray-100 py-4">
            ...
            {menuData.length > 0 ? (
              menuData.map((column, index) => {
                const Icon = getCategoryIcon(column.title);
        ...
        >
          ...
          <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100 shrink-0">
            <div>
              <p className="text-[15px] font-bold text-gray-900">Browse Exams</p>
