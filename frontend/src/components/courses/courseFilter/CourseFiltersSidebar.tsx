  ...
  return (
    <div className="relative mb-2">
      ...
      <input
        ...
        className="w-full rounded-lg border border-slate-200 bg-slate-50 py-1.5 pl-8 pr-3 text-xs text-slate-700 placeholder:text-slate-400 focus:border-[#635dc1] focus:outline-none focus:ring-1 focus:ring-[#635dc1]"
      />
      {value && (
        <button
          onClick={() => onChange("")}
...
const CourseFiltersSidebar: React.FC<CourseFiltersSidebarProps> = ({
  ...
  useEffect(() => {
    const fetchFilters = async () => {
      try {
        ...
        const rawStreams = Array.isArray(streamsData) ? streamsData : streamsData?.data || [];
        const rawModes = Array.isArray(modesData) ? modesData : modesData?.data || [];
        const rawStates = (locationData.states || []).map((s: string) => ({ _id: s, name: s }));
        const rawCities = (locationData.cities || []).map((c: string) => ({ _id: c, name: c }));

        setStreams([...rawStreams].sort(sortByName));
        setProgramModes([...rawModes].sort(sortByName));
        setStates([...rawStates].sort(sortByName));
        setCities([...rawCities].sort(sortByName));
      } catch (error) {
        console.error("Failed to load course filters", error);
      } finally {
        setLoading(false);
  ...
  const filterOptions = (
    ...
    return [
      ...filtered.filter((o) => selectedValues.includes(o._id)),
      ...filtered.filter((o) => !selectedValues.includes(o._id)),
    ];
  };

  // Renders ONLY the checkbox items — no search bar inside
  const renderCheckboxItems = (
    ...
    if (!options.length) return <p className="text-xs text-slate-500 px-1 py-2">No options available yet.</p>;

    const filtered = filterOptions(options, searchValue, selectedValues);

    if (filtered.length === 0) return (
      <p className="text-xs text-slate-400 px-1 py-2">No matches found.</p>
    );
          ...
          <label
            ...
            className={`flex cursor-pointer items-center gap-2 rounded-lg px-3 py-2 text-sm transition
              ${selectedValues.includes(option._id)
                ? "bg-[#ede9fe] text-[#4c1d95]"
                : "bg-slate-50 text-slate-700 hover:bg-[#ede9fe]"
              }`}
          >
            <input
              type="checkbox"
  ...
  // Mobile drawer section — search bar is ABOVE the scrollable area
  const DrawerSection = ({
    id,
    label,
    searchValue,
    ...
    <div className="border-b border-gray-100 pb-3">
      ...
      {isOpen(id) && (
        <div className="pt-1">
          {/* Search bar — outside scroll, always visible */}
    ...
    <>
      ...
      <div className="lg:hidden">
        ...
        `}>
          ...
          <div className="flex-1 overflow-y-auto px-4 py-3" style={{ scrollbarWidth: "none" }}>
            <div className="space-y-1">
              <DrawerSection id="stream" label="Stream" searchValue={streamSearch} onSearchChange={setStreamSearch} searchPlaceholder="Search streams...">
                {renderCheckboxItems(streams, selectedStreams, (v) => toggleSelection(selectedStreams, onStreamsChange, v), streamSearch)}
              </DrawerSection>
              <DrawerSection id="state" label="State" searchValue={stateSearch} onSearchChange={setStateSearch} searchPlaceholder="Search states...">
                {renderCheckboxItems(states, selectedStates, (v) => toggleSelection(selectedStates, onStatesChange, v), stateSearch)}
              </DrawerSection>
              <DrawerSection id="city" label="City" searchValue={citySearch} onSearchChange={setCitySearch} searchPlaceholder="Search cities...">
                {renderCheckboxItems(cities, selectedCities, (v) => toggleSelection(selectedCities, onCitiesChange, v), citySearch)}
              </DrawerSection>
              <DrawerSection id="fee" label="Avg Fee / Year">
                {renderRangeOptions(feeRangeOptions, selectedFeeRanges, (v) => toggleSelection(selectedFeeRanges, onFeeRangeChange, v))}
              </DrawerSection>
              <DrawerSection id="type" label="Course Type" searchValue={courseTypeSearch} onSearchChange={setCourseTypeSearch} searchPlaceholder="Search course types...">
                {renderCheckboxItems(programModes, selectedCourseTypes, (v) => toggleSelection(selectedCourseTypes, onCourseTypeChange, v), courseTypeSearch)}
              </DrawerSection>
              <DrawerSection id="duration" label="Course Duration">
                {renderRangeOptions(durationRangeOptions, selectedDurations, (v) => toggleSelection(selectedDurations, onDurationChange, v))}
              </DrawerSection>
      ...
      <aside className="hidden lg:block rounded-2xl border border-slate-100 bg-white p-4 shadow-sm lg:sticky lg:top-24">
        ...
        <div className="space-y-4">
          ...
          <section>
            <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-500">Stream</p>
            {/* Search bar — fixed, outside scroll */}
            <FilterSearch value={streamSearch} onChange={setStreamSearch} placeholder="Search streams..." />
            <div className="max-h-44 overflow-y-auto pr-1" style={{ scrollbarWidth: "thin" }}>
              {renderCheckboxItems(streams, selectedStreams, (v) => toggleSelection(selectedStreams, onStreamsChange, v), streamSearch)}
            </div>
          </section>
          ...
          <section>
            ...
            <FilterSearch value={stateSearch} onChange={setStateSearch} placeholder="Search states..." />
            <div className="max-h-44 overflow-y-auto pr-1" style={{ scrollbarWidth: "thin" }}>
              {renderCheckboxItems(states, selectedStates, (v) => toggleSelection(selectedStates, onStatesChange, v), stateSearch)}
            </div>
          </section>
          ...
          <section>
            ...
            <FilterSearch value={citySearch} onChange={setCitySearch} placeholder="Search cities..." />
            <div className="max-h-44 overflow-y-auto pr-1" style={{ scrollbarWidth: "thin" }}>
              {renderCheckboxItems(cities, selectedCities, (v) => toggleSelection(selectedCities, onCitiesChange, v), citySearch)}
            </div>
          </section>
          ...
          <section>
            <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-500">Avg Fee / Year</p>
            {renderRangeOptions(feeRangeOptions, selectedFeeRanges, (v) => toggleSelection(selectedFeeRanges, onFeeRangeChange, v))}
          </section>

          <section>
            <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-500">Course Type</p>
            <FilterSearch value={courseTypeSearch} onChange={setCourseTypeSearch} placeholder="Search course types..." />
            <div className="max-h-44 overflow-y-auto pr-1" style={{ scrollbarWidth: "thin" }}>
              {renderCheckboxItems(programModes, selectedCourseTypes, (v) => toggleSelection(selectedCourseTypes, onCourseTypeChange, v), courseTypeSearch)}
            </div>
          </section>
          ...
          <section>
            <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-500">Course Duration</p>
            {renderRangeOptions(durationRangeOptions, selectedDurations, (v) => toggleSelection(selectedDurations, onDurationChange, v))}
          </section>
