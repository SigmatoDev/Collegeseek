  ...
  return (
    <div className="sm:hidden space-y-4 animate-pulse">
      ...
      <div className="flex w-full rounded-full border border-orange-100 bg-white overflow-hidden h-10">
        {[0, 1, 2].map((i) => (
          <div key={i} className={`flex-1 mx-1 my-1 rounded-full ${i === 0 ? "bg-orange-200" : "bg-gray-100"}`} />
        ))}
      </div>
    ...
    <div className="hidden sm:block animate-pulse">
      ...
      <div className="grid gap-8 lg:grid-cols-[320px,1fr]">
        ...
        <div className="rounded-3xl border bg-white/90 p-4 shadow-sm space-y-3">
          {[0, 1, 2, 3, 4, 5].map((i) => (
            <div key={i} className="flex items-center gap-4 rounded-2xl border border-orange-100 px-4 py-3">
        ...
        <div className="rounded-[32px] border bg-white/90 p-8 shadow-lg space-y-6">
          ...
          <div className="flex flex-wrap gap-2">
            {[100, 130, 90, 115, 80, 140, 95, 120, 85, 110].map((w, i) => (
              <div key={i} className="h-7 bg-gray-100 rounded-full shadow-sm" style={{ width: w }} />
            ))}
          </div>
        </div>
      </div>
...
export default function CategoryGrid() {
  ...
  const [data, setData] = useState<CategoryData>({ Streams: [], Exams: [], Courses: [] });
  const [tabCounts, setTabCounts] = useState<Record<TabKey, number>>({ Streams: 0, Exams: 0, Courses: 0 });
  const [selectedItem, setSelectedItem] = useState<CategoryItem | null>(null);
  const [relatedColleges, setRelatedColleges] = useState<{ name: string; slug?: string }[]>([]);
  const [collegesLoading, setCollegesLoading] = useState(false);
  const [loading, setLoading] = useState(true);
  const chipScrollRef = useRef<HTMLDivElement>(null);

  const tabKeyMap: Record<TabKey, keyof CategoryData> = { Streams: "Streams", Exams: "Exams", Courses: "Courses" };
  const getCountLabel = (tab: TabKey) => tab === "Courses" ? "courses" : "colleges";
    ...
    const fetchCategoryData = async () => {
      try {
        ...
        const grouped: CategoryData = {
          Streams: list.filter(c => c.type === "streams").map(c => ({ ...c, count: Number(c.count || 0) })),
          Exams: list.filter(c => c.type === "exams").map(c => ({ ...c, count: Number(c.count || 0) })),
          Courses: list.filter(c => c.type === "courses").map(c => ({ ...c, count: Number(c.count || 0) })),
        };
        setData(grouped);
        setTabCounts({
  ...
  useEffect(() => {
    ...
    const fetchColleges = async () => {
      ...
      try {
        const filterKey = activeTab === "Exams" ? "exams" : activeTab === "Courses" ? "categories" : "streams";
        const res = await fetch(`${api_url}get/colleges/filter`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ page: 1, limit: 15, [filterKey]: [selectedItem.name] }),
          signal: controller.signal,
        });
        const response = await res.json();
        const list = response?.colleges || response?.data?.colleges || response?.data?.data || [];
        setRelatedColleges(
  ...
  const buildHref = (item: CategoryItem) => {
    if (activeTab === "Exams") return `/college?page=1&exams=${encodePlus(item.name.toUpperCase())}`;
    if (activeTab === "Courses") return `/college?page=1&categories=${encodePlus(toTitleCase(item.name))}`;
    return `/college?streams=${encodePlus(toTitleCase(item.name))}`;
  };

  const buildCollegePillHref = (c: { name: string; slug?: string }) =>
    c.slug ? `/colleges/${c.slug}` : `/college?search=${encodeURIComponent(c.name)}`;
  ...
  return (
    <section className="bg-gradient-to-b from-orange-50 to-orange-50 py-10 sm:py-16">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <h2 className="font-extrabold text-center text-gray-900 mb-8 sm:mb-12
            ...
            <div className="sm:hidden space-y-4">
                ...
                {currentList.map((item, index) => {
                  ...
                  return (
                    <button
                      ...
                      `}
                    >
                      <span className={`h-6 w-6 shrink-0 rounded-full flex items-center justify-center bg-gradient-to-br ${swatch.bg}`}>
                        <span className={`text-[10px] font-bold ${swatch.text}`}>{item.name.charAt(0)}</span>
                      </span>
                      <span className={`text-xs font-semibold whitespace-nowrap ${isSelected ? "text-[#d35e45]" : "text-gray-700"}`}>
                        {item.name}
                      </span>
                      {isSelected && (
              ...
              {selectedItem && (
                <div className="rounded-2xl border border-orange-100 bg-white/95 shadow-lg p-4 space-y-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <h3 className="text-lg font-extrabold text-gray-900">{selectedItem.name}</h3>
                      <p className="text-xs text-gray-500 mt-0.5">
                        {selectedItem.count} {getCountLabel(activeTab)} available
                      </p>
                    </div>
                    <Link
                  ...
                  <div>
                    <p className="text-[10px] uppercase tracking-widest text-gray-400 font-semibold mb-2">Colleges</p>
                    <div className="flex flex-wrap gap-1.5">
                      {collegesLoading ? (
                        /* College pills loading skeleton */
                        <div className="flex flex-wrap gap-1.5 w-full animate-pulse">
                          {[90, 120, 80, 110, 95, 105, 75, 130].map((w, i) => (
                            <div key={i} className="h-6 bg-orange-100 rounded-full" style={{ width: w }} />
                          ))}
                        </div>
                        ...
                        <span className="text-xs text-gray-400">No colleges found.</span>
                      )}
                    </div>
                  </div>
                </div>
                      ...
                      return (
                        <button
                          ...
                          className={`flex w-full items-center gap-4 rounded-2xl border px-4 py-3 ${
                            isSelected ? "border-[#d35e45] bg-[#fff0e6]" : "border-orange-100"
                          }`}
                        >
                          <div className={`h-12 w-12 flex items-center justify-center rounded-xl bg-gradient-to-br ${swatch.bg}`}>
                            <span className={`text-base font-bold ${swatch.text}`}>{item.name.charAt(0)}</span>
                          </div>
                          <div>
                            <p className="text-sm font-semibold text-left">{item.name}</p>
                            <p className="text-xs text-gray-500 flex justify-start">{item.count} {getCountLabel(activeTab)}</p>
                          </div>
                        </button>
                      );
                    })}
                  ...
                  {selectedItem ? (
                    <>
                      ...
                      <div className="flex gap-4 flex-wrap">
                        <Link href={buildHref(selectedItem)} className="bg-[#d25c40] text-white px-6 py-2 rounded-full shadow">
                          View Colleges
                        </Link>
                        <Link href="/contactUs" className="border border-[#d25c40] text-[#d25c40] px-6 py-2 rounded-full">
                          Get Admission Help
                        </Link>
                      </div>
                      <div className="flex flex-wrap gap-2">
                        {collegesLoading ? (
                          <div className="flex flex-wrap gap-2 animate-pulse">
                            {[100, 130, 90, 115, 80, 140, 95, 120].map((w, i) => (
                              <div key={i} className="h-7 bg-gray-100 rounded-full" style={{ width: w }} />
                            ))}
                          </div>
                        ) : relatedColleges.length > 0 ? (
                          relatedColleges.map((college, i) => (
                            <Link key={i} href={buildCollegePillHref(college)} className="bg-white px-3 py-1 rounded-full text-xs shadow">
                              {college.name}
                            </Link>
                          ))
                        ) : (
                          <span className="text-xs text-gray-500">No colleges found.</span>
                        )}
                      </div>
                    </>
                  ) : (
