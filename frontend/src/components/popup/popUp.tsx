...
const PopUp = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
      ...
      {/* ══════════════════════════════════════════
          MOBILE modal — full screen centered form
      ══════════════════════════════════════════ */}
      <div className="md:hidden w-full max-w-sm mx-auto bg-white rounded-3xl shadow-2xl overflow-y-auto max-h-[95vh] p-6 relative">

        {/* Close button */}
        <button
          onClick={handleClose}
          className="absolute top-4 right-4 text-gray-600 hover:text-[#D35B42] transition p-1 rounded-full hover:bg-gray-100 z-10"
        >
        ...
        <div className="space-y-1 mt-2">
          <div className="h-1 w-12 rounded-full" style={{ background: `linear-gradient(to right, ${colors.accent.orange}, ${colors.accent.red})` }} />
          <h2 className="text-2xl font-bold" style={{ color: colors.primary.dark }}>Create an Account</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-3 mt-5">
          ...
          <div className="space-y-1">
            ...
            <input
              ...
              className="w-full px-4 py-3 text-sm bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#e35235]/30 focus:border-[#e35235] transition placeholder:text-gray-400"
            />
          </div>
            ...
            <div className="space-y-1">
              ...
              <div className="relative">
                <input
                  ...
                  onChange={handleInputChange}
                  className="w-full px-3 pr-8 py-3 text-sm bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#e35235]/30 focus:border-[#e35235] transition placeholder:text-gray-400"
                />
                <button
                  type="button"
          ...
          <button
            type="submit"
            className="w-full py-3.5 rounded-2xl text-sm font-bold text-white shadow-lg active:scale-[0.98] transition-all duration-300 hover:-translate-y-0.5 mt-2"
            style={{
        ...
        <p className="text-center text-xs text-gray-400 mt-4">
          ...
          <Link href="/user/auth/logIn" className="font-semibold hover:underline" style={{ color: colors.accent.red }}>
            Login here
          </Link>
        </p>
        ...
        {/* Messages */}
        {error && (
          <p className="text-xs text-center text-red-600 bg-red-50 border border-red-100 rounded-xl px-3 py-2.5 mt-3">{error}</p>
        )}
        {success && (
          <p className="text-xs text-center text-green-600 bg-green-50 border border-green-100 rounded-xl px-3 py-2.5 mt-3">{success}</p>
        )}
      </div>

      {/* ══════════════════════════════════════════
          DESKTOP modal — image + form side by side
      ══════════════════════════════════════════ */}
      <div className="hidden md:flex w-full max-w-4xl bg-white rounded-2xl shadow-2xl overflow-hidden relative">
        
        {/* Close button */}
        <button
          onClick={handleClose}
          className="absolute top-4 right-4 z-10 text-gray-600 hover:text-[#D35B42] transition p-1 rounded-full hover:bg-gray-100"
        >
        ...
        <div className="w-1/2 relative">
          <Image
            ...
            style={{ objectPosition: "-220px", transform: "scaleX(-1)" }}
            width={800}
            height={600}
          />
          <div className="absolute inset-0" style={{ background: `linear-gradient(to right, ${colors.primary.dark}20, transparent)` }} />
        </div>

        {/* Right side - Form */}
        <div className="w-1/2 p-8 flex flex-col justify-center">
          <div className="mb-6">
            <div className="h-1 w-12 rounded-full mb-3" style={{ background: `linear-gradient(to right, ${colors.accent.orange}, ${colors.accent.red})` }} />
            <h2 className="text-2xl font-bold" style={{ color: colors.primary.dark }}>Create an Account</h2>
            <p className="text-sm text-gray-500">Join us and start your journey</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            <div>
              ...
              <input
                type="text"
                name="name"
                className="w-full px-4 py-2.5 text-sm bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#e35235]/30 focus:border-[#e35235] transition placeholder:text-gray-400"
                placeholder="Enter your full name"
                value={registerData.name}
                onChange={handleInputChange}
              />
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <div className="relative">
                  <input
                    type={showPassword ? "text" : "password"}
                    name="password"
                    className="w-full px-3 pr-8 py-2.5 text-sm bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#e35235]/30 focus:border-[#e35235] transition placeholder:text-gray-400"
                    placeholder="••••••••"
                    value={registerData.password}
                    onChange={handleInputChange}
                  />
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1">Confirm</label>
                <div className="relative">
                  <input
                    type={showConfirmPassword ? "text" : "password"}
                    name="confirmPassword"
            ...
            <button
              type="submit"
              className="w-full py-3 rounded-xl text-sm font-bold text-white shadow-lg active:scale-[0.98] transition-all duration-300 hover:-translate-y-0.5 mt-1"
              style={{
                background: `linear-gradient(to right, ${colors.accent.orange}, ${colors.accent.red})`,
                boxShadow: `0 8px 25px ${colors.accent.red}40`
              }}
          ...
          {(error || success) && (
            <div className="mt-3">
              {error && (
                <p className="text-xs text-center text-red-600 bg-red-50 border border-red-100 rounded-xl px-3 py-2">{error}</p>
              )}
              {success && (
